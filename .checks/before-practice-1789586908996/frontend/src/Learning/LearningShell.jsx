...
export default function LearningShell({ children }) {
  ...
  return <div className="min-h-screen bg-[#f5f6fb] text-slate-900">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-5 backdrop-blur"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 py-4"><Link to="/learn" className="text-xl font-black tracking-tight">Code<span className="text-purple-700">Rwanda</span> <span className="text-sm font-medium text-slate-400">/ Learn</span></Link><nav className="flex items-center gap-4 text-sm font-semibold"><Link to="/learn">My learning</Link><Link to="/training-room">Explore courses</Link>{auth.data?.admin_access ? <Link to="/admin/learning">Admin</Link> : null}<button onClick={async () => { try { await api.post('/auth/logout'); } catch { /* Clear the local session when offline too. */ } clearSession(); window.location.assign('/login?redirect=/learn'); }}>Sign out</button></nav></div></header>
