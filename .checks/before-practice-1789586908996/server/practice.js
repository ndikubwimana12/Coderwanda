...
function files(value, language) {
  const names = language === 'web' ? ['index.html', 'style.css', 'main.js'] : ['main.js'];
...
function exerciseBody(body) {
  ...
  if (!['javascript', 'web'].includes(language)) fail('Choose JavaScript or Web.');
...
function registerPractice(app) {
  ...
  app.post('/api/practice/exercises', async (req, res) => {
    ...
    const data = exerciseBody(req.body); const values = { unit_id: parent.id, ...data };
  ...
  app.post('/api/practice/exercises/:id/submit', async (req,res) => {
    ...
    const id = await transaction(async conn => {
      await conn.query('SELECT id FROM coding_exercises WHERE id=? FOR UPDATE',[exercise.id]);
