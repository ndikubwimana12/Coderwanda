...
async function assessmentAccess(conn, user, assessment) {
  ...
  } else if (!(await outline(conn, user.id, assessment.course_id)).ready) fail('Pass every published unit assessment before the final exam.', 403);
}
