  ...
  return (
    ...
    >
      <div className="flex h-14 items-center gap-3 px-4 sm:px-5">
        ...
        {/* Search */}
        <form onSubmit={event => { event.preventDefault(); const match = Object.entries(titles).find(([route, label]) => label.toLowerCase().includes(search.toLowerCase()) || route === search); if (match) { navigate(match[0]); setSearch(''); } }} className="relative flex-1 max-w-xs ml-4 hidden md:block">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input
            type="text"
            aria-label="Find dashboard page" list="dashboard-pages" value={search} onChange={event => setSearch(event.target.value)} placeholder="Find dashboard page…"
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1">
            ...
            <span className="text-[11px] font-bold text-emerald-600">{health.loading ? "Checking" : health.error ? "Offline" : "Online"}</span>
          </div>
