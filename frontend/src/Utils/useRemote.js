...
export default function useRemote(path, initial = EMPTY) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('focus', reload); window.removeEventListener('contentChanged', reload); window.removeEventListener('authChanged', reload); window.removeEventListener('storage', onStorage); };
  }, [reload]);
