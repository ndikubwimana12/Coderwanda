...
async function authenticate(req, res, next) {
  ...
  const [rows] = await pool.query(`SELECT u.id, u.name, u.email, u.phone, u.role, r.admin_access, EXISTS(SELECT 1 FROM course_trainers t WHERE t.user_id=u.id) AS is_trainer FROM sessions s JOIN users u ON u.id=s.user_id JOIN roles r ON r.name=u.role WHERE s.token_hash=? AND s.expires_at > NOW() AND u.account_active=1`, [hash(token)]);
