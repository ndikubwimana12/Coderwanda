...
async function main() {
  ...
  try {
    ...
    const click = async text => { await until(() => evaluate(`Array.from(document.querySelectorAll('button')).some(item => item.textContent.trim() === ${JSON.stringify(text)} && !item.disabled)`), 'Enabled button: ' + text); assert.ok(await evaluate(`(() => { const button = Array.from(document.querySelectorAll('button')).find(item => item.textContent.trim() === ${JSON.stringify(text)}); if (!button) return false; button.click(); return true; })()`), 'Button: ' + text); };
    ...
    await fill('[aria-label="Code editor main.js"]','while (true) {}');await click('Run & test');
